...
export default function Catalog({ onSelectProduct, onAddToCart, cartItemIds, searchFilter, setSearchFilter }) {
  ...
  return (
    <section id="catalog" className="py-16 bg-cream-50 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Title */}
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-10">
          <span className="inline-block text-xs font-extrabold text-amber-800 tracking-widest uppercase bg-amber-100 px-3.5 py-1 rounded-full border border-amber-200">
            Catalog Collection
          </span>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-extrabold text-wood-950">
            Sheesham & Neem Wood Handicrafts
          </h2>
          <p className="text-wood-600 text-sm sm:text-base font-medium">
            Explore our complete WhatsApp store collection. Order individual items or build a custom order list to send directly on WhatsApp.
          </p>
        </div>

        {/* Filters & Search Bar Header */}
        <div className="bg-white border border-wood-200 rounded-2xl p-4 sm:p-6 mb-10 shadow-soft-card space-y-4">
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-200 ${
                  selectedCategory === cat
                    ...
                    : 'bg-cream-100 text-wood-800 hover:bg-cream-200 border border-wood-200'
                }`}
              >
                {cat}
              </button>
          ...
          {/* Search bar & Sorting */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-3 border-t border-wood-100">
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full bg-cream-100 border border-wood-200 focus:border-wood-800 rounded-xl pl-10 pr-4 py-2 text-xs sm:text-sm text-wood-950 placeholder-wood-500 focus:outline-none transition-colors font-medium"
              />
            </div>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
              ...
              <div className="relative">
                <select
                  ...
                  className="bg-cream-100 border border-wood-200 text-xs text-wood-800 font-bold rounded-xl px-3 py-2 pr-8 focus:outline-none focus:border-wood-800 appearance-none cursor-pointer"
                >
                  <option value="recommended">Featured Catalog Order</option>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="group bg-white border border-wood-200 hover:border-wood-400 rounded-2xl overflow-hidden shadow-soft-card hover:shadow-soft-hover transition-all duration-300 flex flex-col justify-between"
                >
                  <div>
                    {/* Image Container */}
                    <div className="relative h-56 overflow-hidden bg-cream-100">
                      ...
                      <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                        <span className="bg-wood-900 text-white text-[10px] font-extrabold px-2.5 py-1 rounded-md shadow uppercase tracking-wider">
                          {product.tag}
                        </span>
                      </div>
                      ...
                      <button
                        ...
                        className="absolute bottom-3 right-3 bg-white/90 text-wood-900 hover:text-black p-2.5 rounded-xl border border-wood-200 backdrop-blur-md opacity-90 sm:opacity-0 group-hover:opacity-100 transition-all duration-200 shadow-md"
                    ...
                    <div className="p-5 space-y-2.5">
                      <div className="flex items-center justify-between text-xs text-wood-600 font-semibold">
                      ...
                      <div className="text-[11px] text-wood-800 font-bold bg-cream-200 px-2.5 py-1 rounded-md inline-block border border-wood-200">
                        🪵 {product.material}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 pt-3 border-t border-wood-100 bg-cream-50/50 flex flex-col gap-2.5">
          ...
          <div className="text-center py-16 bg-white border border-wood-200 rounded-2xl shadow-sm">

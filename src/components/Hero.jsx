...
export default function Hero({ onOpenCustomQuote }) {
  return (
    <div className="relative overflow-hidden bg-royalDark pt-8 pb-16 lg:pt-16 lg:pb-28">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gold-600/10 rounded-full blur-[140px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
            ...
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-gold-950/80 border border-gold-600/40 text-gold-300 text-xs font-semibold tracking-wider uppercase shadow-gold-glow">
              ...
              <span>Moradabad 'Pital Nagri' Master Craftsmen</span>
            </div>

            {/* Main Headline */}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-bold text-white tracking-tight leading-[1.15]">
              Timeless Royal Brassware & <br />
              <span className="text-gold-gradient font-normal italic">Bespoke Metal Artistry</span>
            </h1>

            {/* Description */}
            <p className="text-base sm:text-lg text-gray-300 max-w-2xl mx-auto lg:mx-0 font-light leading-relaxed">
              Discover authentic Moradabad handcrafted brass urlis, Moroccan lanterns, Islamic calligraphy wall art, and royal tableware crafted by generational artisans.
            </p>

            {/* Metric Highlights */}
            <div className="grid grid-cols-3 gap-4 pt-2 max-w-md mx-auto lg:mx-0 border-y border-royalBorder py-4">
              <div>
                <span className="block text-2xl font-serif font-bold text-gold-royal">500+</span>
                <span className="text-xs text-gray-400">Handmade Designs</span>
              </div>
              <div>
                <span className="block text-2xl font-serif font-bold text-gold-royal">100%</span>
                <span className="text-xs text-gray-400">Pure Brass Metal</span>
              </div>
              <div>
                <span className="block text-2xl font-serif font-bold text-gold-royal">Global</span>
                <span className="text-xs text-gray-400">Export & Wholesale</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center lg:justify-start gap-6 text-xs text-gray-400 pt-3">
              <span className="flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Direct Wholesale Pricing
              </span>
              <span className="flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-gold-400" /> Quality Certified Metalware
              </span>
            </div>
          ...
          <div className="lg:col-span-5 relative">
            ...
            <div className="relative mx-auto max-w-md lg:max-w-none">
              <div className="absolute -inset-1 bg-gradient-to-r from-gold-600 via-amber-500 to-gold-700 rounded-3xl blur-lg opacity-40 animate-pulse-slow"></div>
              
              <div className="relative rounded-2xl bg-royalCard border border-gold-600/30 overflow-hidden shadow-2xl">
                <img
                  src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1000&q=80"
                  alt="Royal Peacock Brass Urli Bowl"
                  className="w-full h-80 sm:h-96 object-cover transform hover:scale-105 transition-transform duration-700"
                />
                
                {/* Overlay Details */}
                <div className="absolute inset-0 bg-gradient-to-t from-royalDark via-royalDark/30 to-transparent flex flex-col justify-end p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-xs text-gold-300 uppercase tracking-widest font-semibold block">Artisan Spotlight</span>
                      <h3 className="text-xl font-serif font-bold text-white">Royal Peacock Hand-Carved Urli</h3>
                    </div>
                  ...
                  <p className="text-xs text-gray-300 mt-2 line-clamp-2">
                    Hand-chiselled solid brass decorative urli for floating flowers, candles, & grand welcoming decor.
                  </p>
                  
                  <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between">
                    <span className="text-gold-gradient font-bold text-lg">₹4,850 / $65</span>
                    <a
                      href={WHATSAPP_CATALOG_URL}
                      target="_blank"
                      rel="noreferrer"
              ...
              <div className="absolute -bottom-6 -left-6 bg-royalCard/90 backdrop-blur-md border border-gold-500/40 p-4 rounded-xl shadow-xl flex items-center gap-3 max-w-xs">
                <div className="w-10 h-10 rounded-lg bg-gold-900/50 border border-gold-600/50 flex items-center justify-center shrink-0">
                  <Flame className="w-5 h-5 text-gold-royal" />
                </div>
                <div>
                  <h4 className="text-xs font-bold text-white">Custom Manufacturing</h4>
                  <p className="text-[11px] text-gray-400">OEM & bulk production for hotels, events, & export</p>
                </div>
              </div>
